...
// const displayNames = i.map(user => i.displayName);
// console.log(displayNames);

const employeeData = users.map(
  (i) => (
    i.displayName, i.employeeNumber, i.jobtitle)
);
console.log(employeeData);

const seniorEngineers = users.filter(
  (user) => user.jobtitle === "Sr. Front-End Engineer"
);
console.log(seniorEngineers);

const employeeNumber = users.reduce(
  (accumulator, currentValue) => accumulator + currentValue.employeeNumber,
  0
);
console.log(`sum of employeeNumber of all users ${employeeNumber}`);
